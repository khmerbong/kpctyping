from flask import Flask, render_template, request, jsonify, send_from_directory, session, g, redirect, send_file
...
def ensure_user_tables():
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS ai_key_stats (
            ...
            UNIQUE(user_id, guest_id, key_value),
            FOREIGN KEY(user_id) REFERENCES users(id)
        )
    """)
    conn.execute("""
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS race_players (
            ...
            updated_at TEXT NOT NULL,
            finished_at TEXT,
            UNIQUE(room_id, user_id, guest_id),
...
def current_user():
    ...
    if not uid:
        return None
    conn = get_db_connection()
    user = conn.execute("SELECT id, username, email, xp, created_at, last_login FROM users WHERE id = ?", (uid,)).fetchone()
    conn.close()
    return dict(user) if user else None

@app.context_processor
def inject_current_user():
    return {"current_user": current_user(), "csrf_token_value": session.get("csrf_token", "")}
...
def clean_game(value):
    allowed = {"speed", "zombie", "vampire", "space", "all"}
    return value if value in allowed else "all"

@app.route("/")
def home():
    # Home screen opens directly to playable Academy / Training Mode.
    # The old marketing landing page is still available at /landing.
...
def submit_score():
    ...
    raw_name = str(data.get("player_name") or "Player").strip()
    # Keep names friendly and safe: letters, numbers, Khmer unicode, spaces, _ and - only.
    player_name = re.sub(r"[^0-9A-Za-z_\-\s\u1780-\u17FF]", "", raw_name)[:24].strip() or "Player"
...
def register():
    ...
    try:
        ...
        cur = conn.execute(
            ...
            (username, email or None, password_hash, now, now)
        )
        conn.commit()
...
def profile():
    ...
    progress_rows = conn.execute(
        "SELECT progress_type, payload, updated_at FROM user_progress WHERE user_id = ? ORDER BY updated_at DESC",
        (user["id"],)
    ).fetchall()
    conn.close()
    return render_template("profile.html", user=user, progress_rows=progress_rows)
...
def _build_ai_coach_summary(rows):
    ...
    if weak_keys:
        base_keys = [k["key"] for k in weak_keys[:5]]
        base = " ".join(base_keys)
        mirror = " ".join(reversed(base_keys))
        recommended = " ".join(([base, mirror, "slow clean rhythm", base.lower(), "accuracy before speed"] * 2)).strip()
        if accuracy < 90:
            coach_message = "Your next win is accuracy. Type slower, clean up weak keys, then increase speed."
        elif total_slow:
            coach_message = "Accuracy is improving. Now train rhythm on slow keys so your speed becomes smoother."
        else:
            coach_message = "Practice your weak keys slowly before increasing speed."
    else:
        recommended = "f j d k s l a ; f j d k s l a ; clean rhythm clean accuracy"
        coach_message = "Start with home-row accuracy. The coach will adapt after you type."
    readiness = "starter"
    if total_keys >= 1200 and accuracy >= 96:
        readiness = "elite"
    elif total_keys >= 600 and accuracy >= 94:
        readiness = "advanced"
    elif total_keys >= 250:
    ...
    return {
        ...
        "readiness": readiness,
        "weekly_goal": "Reduce top weak-key errors by 20% and keep accuracy above 95%.",
    }
    ...
    with get_db_connection() as conn:
        for ev in events:
            ...
            conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(user_id, guest_id, key_value)
                DO UPDATE SET
    ...
    with get_db_connection() as conn:
        for _ in range(10):
            ...
            try:
                ...
                conn.commit()
                return jsonify({"ok": True, "room": _race_room_payload(conn, code)})
            except sqlite3.IntegrityError:
                continue
...
def api_race_join():
    ...
    with get_db_connection() as conn:
        ...
        conn.execute("""INSERT OR IGNORE INTO race_players (room_id, user_id, guest_id, player_name, joined_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)""", (room["id"], uid, guest_id, name, now, now))
        conn.commit()
        return jsonify({"ok": True, "room": _race_room_payload(conn, code)})
...
def _leaderboard_query(conn, where="", params=(), limit=100):
    query = """
        SELECT username, country, game_mode, MAX(score) AS score, MAX(wpm) AS wpm,
               MAX(accuracy) AS accuracy, MAX(xp) AS xp, career_rank, season_key, week_key, month_key,
               MAX(created_at) AS created_at
        FROM global_leaderboard_scores
        WHERE verified = 1 {where}
        GROUP BY LOWER(username), country
        ORDER BY wpm DESC, accuracy DESC, score DESC, xp DESC
        LIMIT ?
    """.replace("{where}", where)
    rows = conn.execute(query, (*params, limit)).fetchall()
    return [_leaderboard_score_row(row, idx + 1) for idx, row in enumerate(rows)]
...
def _leaderboard_career_snapshot(conn, user_id):
    ...
    try:
        summary = _career_summary(conn, user_id)
        return {"xp": int(summary.get("total_xp", 0)), "career_rank": summary.get("rank", {}).get("name", "Beginner")}
    except Exception:
...
def _update_hall_of_fame(conn, user_id, username, country, wpm, accuracy, score):
    ...
    for record_type, value in records:
        existing = conn.execute("SELECT value FROM hall_of_fame_records WHERE record_type=? AND user_id IS ?", (record_type, user_id)).fetchone()
        if existing is None:
            conn.execute("INSERT INTO hall_of_fame_records (record_type, user_id, username, country, value, meta_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)", (record_type, user_id, username, country, float(value), json.dumps({"score": score, "wpm": wpm, "accuracy": accuracy}), now))
        elif float(value) > float(existing["value"] or 0):
            conn.execute("UPDATE hall_of_fame_records SET username=?, country=?, value=?, meta_json=?, created_at=? WHERE record_type=? AND user_id IS ?", (username, country, float(value), json.dumps({"score": score, "wpm": wpm, "accuracy": accuracy}), now, record_type, user_id))
...
def api_submit_global_score():
    ...
    now = datetime.utcnow()
    uid = session.get("user_id")
    username = sanitize_player_name(data.get("username") or session.get("username") or "Guest")
    country = _safe_country(data.get("country") or "Global")
    game_mode = re.sub(r"[^a-zA-Z0-9_-]", "", (data.get("game_mode") or "training"))[:30] or "training"
    wpm = float(_safe_int(data.get("wpm"), 0, 0, 300))
    ...
    elif total_keys and wpm > 0 and total_keys < max(10, int(wpm * 2)):
        verified, reason = 0, "key_count_suspicious"
    keys = _phase13_period_keys(now)
    evidence = f"{uid}|{username}|{country}|{game_mode}|{wpm}|{accuracy}|{score}|{duration_sec}|{total_keys}|{now.date()}"
    evidence_hash = hashlib.sha256(evidence.encode("utf-8")).hexdigest()
    with get_db_connection() as conn:
        existing = conn.execute("SELECT id FROM global_leaderboard_scores WHERE evidence_hash=?", (evidence_hash,)).fetchone()
        if existing:
            _record_leaderboard_audit(conn, uid, username, "submit_global_score", "rejected", "duplicate_score", data)
            conn.commit()
            return jsonify({"ok": False, "error": "duplicate_score"}), 409
        ...
        _record_leaderboard_audit(conn, uid, username, "submit_global_score", "accepted" if verified else "flagged", reason, {**data, **keys})
        conn.commit()
    return jsonify({"ok": True, "verified": bool(verified), "reason": reason, "keys": keys})
...
def _user_brief(conn, user_id):
    ...
    return {
        "id": row["id"],
        "username": row["username"],
        "xp": int(career.get("total_xp", row["xp"] or 0)),
        "level": career.get("level", 1),
        "career_rank": (career.get("rank") or {}).get("name", "Beginner"),
        "best_wpm": round(float((best or {}).get("best_wpm") or 0), 2) if best else 0,
        "best_accuracy": round(float((best or {}).get("best_accuracy") or 0), 2) if best else 0,
...
def api_challenge_create():
    ...
    with get_db_connection() as conn:
        ...
        conn.execute("INSERT INTO private_challenges (challenger_id, receiver_id, mode, prompt_text, status, created_at) VALUES (?, ?, ?, ?, 'pending', ?)", (uid, target["id"], mode, prompt_text, now))
        challenge_id = conn.execute("SELECT last_insert_rowid() AS id").fetchone()["id"]
...
def ensure_tournament_tables():
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS tournament_players (
            ...
            UNIQUE(tournament_id, user_id, guest_id),
...
def _current_player_name():
    user = current_user()
    if user:
        return sanitize_player_name(user.get('username')) or 'Player'
...
def api_tournament_join():
    ...
    guest_id = sanitize_player_name(data.get('guest_id') or request.cookies.get('guest_id') or '')[:60] or None
    user = current_user()
...
def api_health():
    """Lightweight health check for Render/UptimeRobot/deploy smoke tests."""
    return jsonify({
        "ok": True,
        "status": "healthy",
        "app": "KPCTyping",
